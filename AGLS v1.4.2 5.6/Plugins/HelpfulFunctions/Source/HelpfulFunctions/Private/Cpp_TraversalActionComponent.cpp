...
void UCpp_TraversalActionComponent::CallOnTraversalDeleage(bool bActionStarted, float Duration)
{
	OnTraversalActionChanged.Broadcast(bActionStarted, Duration);
}
...
bool UCpp_TraversalActionComponent::CheckTraversalActionFast_Implementation(FTraversalCollisionCheck& CheckingResult, FTraversalTraceSettings TraceSettings, ETraceTypeQuery TraceChannel)
{
	...
	//Step 1
	const float TargetReachDistance = TraceSettings.ReachDistance + GetReachOffset();
	...
	FVector InitialTraceImpact = FVector::ZeroVector; FVector InitialTraceNormal = FVector::ZeroVector; FVector DownTraceLoc = FVector::ZeroVector; //Init Main Vectors
	FVector TraceStart = (GetLocationFromBase(2.0) + GetPlayerMovementInput() * -30.0) + FVector(0,0,VerticalCapsuleOffset);
	FVector TraceEnd = TraceStart + (GetPlayerMovementInput() * TargetReachDistance);
	...
	for (int i = 0; i < TraceSettings.MaxForwarIterations; i++)
	{
		...
		float AngleToFloor = KML::RadiansToDegrees(HLF::GetAngleBetween(LedgeHitResult.ImpactNormal, FVector(0, 0, 1)));

		if (LedgeHitResult.bBlockingHit == true && AngleToFloor < 65.0)
		{
			HitComponent = LedgeHitResult.GetComponent(); DownTraceLoc = LedgeHitResult.ImpactPoint;
			break;
		}
	}
	if (LedgeHitResult.bBlockingHit == false || LedgeHitResult.bStartPenetrating == true) { return false; }

	// AGLS v1.3
	//Sprawdzenie czy konieczne jest podjecie próby korekcji pozycji Z punktu krawêdzi. Taka korekta jest przeprowadzania w przypadku kiedy HitNormal zwraca du¿e nachylenie wzglêdem osi Z
	if (KML::RadiansToDegrees(HLF::GetAngleBetween(LedgeHitResult.ImpactNormal, FVector(0, 0, 1))) > 5.0)
	{
		FHitResult CorrectLedgeHit;
		float MaxPositionZ = DownTraceLoc.Z;
		const FVector AngledDirection = KML::Cross_VectorVector(KML::GetRightVector(KML::MakeRotFromX(HLF::NormalToVector(InitialTraceNormal))), LedgeHitResult.ImpactNormal);

		//DrawDebugLine(GetWorld(), LedgeHitResult.ImpactPoint, LedgeHitResult.ImpactPoint + (AngledDirection * 40), FColor::Green, false, 0.2, 0, 2);

		const bool HitBoxValid = UKismetSystemLibrary::SphereTraceSingle(
			RefChar, LedgeHitResult.ImpactPoint + (AngledDirection * 25),
			LedgeHitResult.ImpactPoint + (AngledDirection * 1) + FVector(0, 0, 0),
			3.0, 
			TraceChannel,
			false,
			ActorsToIgnoreList,
		...
		if (HitBoxValid && abs(CorrectLedgeHit.ImpactPoint.Z - LedgeHitResult.ImpactPoint.Z) < 20.0)
		{
			DownTraceLoc = FVector(DownTraceLoc.X, DownTraceLoc.Y, CorrectLedgeHit.ImpactPoint.Z);

			if (TraceSettings.RequireWalkableFloor)
			{
				if (WALKABLE(CorrectLedgeHit) == false) { return false; }
			}
		}
	}
	//DrawDebugSphere(GetWorld(), DownTraceLoc, 5.0, 12, FColor::Red, false, 0.03, -1, 0.5);
	//DrawDebugLine(GetWorld(), LedgeHitResult.ImpactPoint, LedgeHitResult.ImpactPoint + (LedgeHitResult.ImpactNormal * 20.0), FColor::Yellow, false, 0.03, -2, 1.2);
	...
	if (HLF::CapsuleHaveRoomWithIgnore(RefChar, RefChar, ToCapsuleLocation(DownTraceLoc, 2.0, UseCapsuleDimensionsFromRef) - FVector(0,0,5), RoomCheckIgnore, 0.94 * CapsuleHaveRoomCheckScale.X, TraceSettings.VerticalCapRoomFindScale, false) == false
		|| KML::Vector_Distance2D(DownTraceLoc, RefChar->GetActorLocation()) > 400)
	{ 
		if (DrawDebugShapes)
		{
			DrawDebugCapsule(GetWorld(), ToCapsuleLocation(DownTraceLoc, 2.0, UseCapsuleDimensionsFromRef) - FVector(0, 0, 5), 
				CAP->GetScaledCapsuleHalfHeight() * TraceSettings.VerticalCapRoomFindScale, CAP->GetScaledCapsuleRadius() * 0.94 * CapsuleHaveRoomCheckScale.X,
				RefChar->GetActorQuat(), FColor::Red, false, NotValidCapsuleCheckTime, -1, 1.3);
		}
		return false; 
	}
	...
	FVector BackTraceImpact = FVector(0, 0, 0); FVector BackTraceNormal = FVector(0, 0, 0);
	...
	if (BackLedgeValid == true && BackLedgeHit.bStartPenetrating == false && WALKABLE(BackLedgeHit) == false)
	{
		...
		else if(HLF::CapsuleHaveRoomWithIgnore(RefChar, RefChar, BackLedgeHit.ImpactPoint + (HLF::NormalToVector(BackLedgeHit.ImpactNormal) * -20.0) + FVector(0,0,-15), ActorsToIgnoreList, 0.25 * CapsuleHaveRoomCheckScale.X, 0.5 * CapsuleHaveRoomCheckScale.Y, false) == true)
		{
			BackTraceImpact = BackLedgeHit.ImpactPoint; BackTraceNormal = BackLedgeHit.ImpactNormal;
			//DrawDebugSphere(GetWorld(), BackTraceImpact, 5.0, 12, FColor::Orange, false, 0.03, -1, 0.5);
			//DrawDebugLine(GetWorld(), BackTraceImpact, BackTraceImpact + (BackTraceNormal * 28.0), FColor::Orange, false, 0.03, -2, 1.0);
		}
		else if(DrawDebugShapes)
		{
			DrawDebugCapsule(GetWorld(), BackLedgeHit.ImpactPoint + (HLF::NormalToVector(BackLedgeHit.ImpactNormal) * -20.0) + FVector(0, 0, -15), 
				CAP->GetUnscaledCapsuleHalfHeight() * 0.5 * CapsuleHaveRoomCheckScale.Y, CAP->GetScaledCapsuleRadius() * 0.25 * CapsuleHaveRoomCheckScale.X,
				RefChar->GetActorQuat(), FColor::Red, false, NotValidCapsuleCheckTime, -1, 1.3);
		}
	}
	...
	if (BackTraceImpact.IsNearlyZero() == false && BackLedgeHit.bStartPenetrating == false)
	{
		...
		{
			if (HLF::CapsuleHaveRoomWithIgnore(RefChar, RefChar, ToCapsuleLocation(WalkableLedgeHit.ImpactPoint, 2.0, UseCapsuleDimensionsFromRef) - FVector(0, 0, 5),
				RoomCheckIgnore, 0.96 * CapsuleHaveRoomCheckScale.X, TraceSettings.VerticalCapRoomFindScale, false) == true)  // <-- Trace Disabled?
			{
			...
			{
				DrawDebugCapsule(GetWorld(), ToCapsuleLocation(WalkableLedgeHit.ImpactPoint, 2.0, UseCapsuleDimensionsFromRef) - FVector(0, 0, 5), TraceSettings.VerticalCapRoomFindScale, 0.96 * CapsuleHaveRoomCheckScale.X, RefChar->GetActorQuat(), FColor::Red, false, NotValidCapsuleCheckTime, -1, 1.3);
			}
		}
	}
...
bool UCpp_TraversalActionComponent::TryActivatePredictableJump_Implementation(ETraceTypeQuery TraceChannel, bool& StartNormalJump)
{
	...
	FTraversalGroundInfo StartGroundInfo; FTraversalGroundInfo LandGroundInfo;
	...
	FTraversalTraceSettings TraceSettings, ETraceTypeQuery TraceChannel, float CapHeightScale, float CapRadiusScale, bool CapSizeByDef, float VelocityOffsetScale, float LedgeFindingRadius, float DebugTime)
{
	...
	//Step 2 - Try find object in front of capsule location
	FVector InitialTraceImpact; FVector InitialTraceNormal = FVector::ZeroVector; FVector DownTraceLoc = FVector::ZeroVector; //Init Main Vectors
...
FVector UCpp_TraversalActionComponent::ToCapsuleLocation(FVector FloorLocation, float ZOffset, bool ByDef)
{
	...
	if (ByDef)
	{
		CapHeight = DefaultCapsuleSize.Y;
	}
