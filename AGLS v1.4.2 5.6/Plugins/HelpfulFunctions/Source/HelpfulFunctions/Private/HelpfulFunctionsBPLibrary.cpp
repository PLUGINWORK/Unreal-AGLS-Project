...
//#include "Math/UnrealMathVectorCommon.h" This file is not exist in UE5.6 Engine
...
FVector UHelpfulFunctionsBPLibrary::InterpolateBetweenPoints(const TArray<FVector>& Points, float Alpha)
{
	...
	if (Points.Num() < 2)
	{
		...
		return FVector::ZeroVector; // Zwróæ domyœlny punkt
	}
	...
	FVector Direction, ETraceTypeQuery TraceChannel, int AccuracyFactor, float MaxRadius, TArray<AActor*> ActorsToIgnore, bool ReturnInLocalSpace, int DrawDebugIndex)
{
	...
	FVector Center = FVector(0,0,0); //Naprawa dla UE5.5 - Nie by³a zadeklarowana wartoœæ bazowa
	float Radius = 0.0;
