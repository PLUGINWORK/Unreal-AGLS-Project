...
void UNavClimbingComponentCore::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (bPrintAditionalInfoOnScreen)
	{
		...
		if (EnumPtr)
		{
			FString EnumValue = EnumPtr->GetNameByValue((int32)NavClimbActionState).ToString();
			FString DisplayName = EnumPtr->GetDisplayNameTextByValue((int32)NavClimbActionState).ToString();
			S1 += DisplayName;
